...
function ImagePlane() {
  ...
  const fragmentShader = `
    ...
    void main() {
      ...
      vec3 mixed = mix(texture.rgb, color, vUv.x);


      float noise = cnoise(vec3(vUv, uTime * 0.2));
      noise = smoothstep(0.0, 0.001, noise);

      mixed = mix(texture.rgb, color, 1. - noise);

      float opacity = 1.;
      float opacityNoise = cnoise(vec3(vUv, uTime * 0.1));
      opacityNoise = smoothstep(0., 0.001, opacityNoise);


      // float edgeDist = length(uMouseUv - vec2(0.5)) * 2.0;
      ...
      edgeDist = floor(edgeDist);
      // gl_FragColor = vec4(color, 1.);
      // gl_FragColor = texture;
      // gl_FragColor = vec4(vec3(noise), 1.);

      mixed = mix(texture.rgb, color, edgeDist);
      gl_FragColor = vec4(mixed, opacity);
      // gl_FragColor = vec4(vec3(edgeDist), 1.);
      // gl_FragColor = vec4(vec3(xNoise), 1.);
    }
  `;
  ...
  useFrame((state) => {
    ...
    const objects = raycaster.intersectObject(ref.current, false);
    if (objects.length > 0) {
      const plane = objects[0];
      ref.current.material.uniforms.uMouseUv.value = plane.uv;
    }
  })
  return <>
    <mesh ref={ref}>
